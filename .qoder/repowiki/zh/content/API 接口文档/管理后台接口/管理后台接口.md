...
## 更新摘要
**变更内容**   
- 重构了管理后台的模块化架构，从单一router.py拆分为pages子包
- 移除了对hushai.meditation.admin.router的直接引用，改为通过build_router()聚合
- 新增共享基础设施模块_shared.py，提供统一的鉴权、模板和工具函数
- 每个资源页面独立成模块，提高代码可维护性和可扩展性
...
graph TB
subgraph "管理后台入口"
A["admin/__init__.py<br/>兼容层，导出 router"] --> B["pages/__init__.py<br/>build_router() 聚合各资源模块"]
end
subgraph "共享基础设施"
C["_shared.py<br/>通用工具、鉴权、模板"]
D["auth.py<br/>管理员登录/鉴权/CSRF"]
E["export.py<br/>CSV/Excel 导出工具"]
end
subgraph "Web 管理页面 (/admin)"
F["auth.py<br/>登录/登出"]
G["dashboard.py<br/>仪表盘"]
H["users.py<br/>用户管理"]
I["conversations.py<br/>对话管理"]
J["memories.py<br/>记忆管理"]
K["knowledge.py<br/>知识管理"]
L["skills.py<br/>技能管理"]
M["audit.py<br/>审计日志"]
N["export.py<br/>数据导出"]
O["settings.py<br/>系统设置"]
P["admin_users.py<br/>管理员用户"]
Q["counseling_dashboard.py<br/>咨询仪表板"]
R["counselors.py<br/>咨询师管理"]
S["appointments.py<br/>预约管理"]
T["orders.py<br/>订单管理"]
U["scenes.py<br/>场景管理"]
end
subgraph "REST 管理 API (/api/admin)"
V["api/admin.py<br/>用户/咨询师/订单/统计"]
end
subgraph "通用能力"
W["db/models.py<br/>ORM 模型"]
X["schemas.py<br/>Pydantic 请求/响应模型"]
end
B --> F
B --> G
B --> H
...
## 核心组件
- **模块化架构**
  - 原 1667 行的 router.py 已按资源拆分到 pages/ 子包
  - 通过 build_router() 函数聚合所有资源模块到一个 APIRouter
  - 保持向后兼容，仍可从 hushai.meditation.admin 直接导入 router
- **共享基础设施**
  - _shared.py 提供统一的模板引擎、鉴权依赖、统计上下文
  - 消除重复的 get_admin_from_request + redirect/HTTPException 样板代码
  - 提供 BatchDeleteRequest、LoginRequest 等通用 Pydantic 模型
- **管理员认证与授权**
...
participant PagesInit as "pages/__init__.py"
...
Router->>PagesInit : build_router() 聚合路由
PagesInit->>Shared : 调用 require_admin_page()
Shared->>Auth : get_admin_from_request()
Auth-->>Shared : 管理员用户名或空
alt 未登录
Shared-->>Router : HTTPException 401
...
### 模块化架构设计
**新增** 管理后台采用全新的模块化架构，将原来单一的 router.py 拆分为独立的资源模块：

```mermaid
flowchart TD
Init["admin/__init__.py<br/>兼容层"] --> Build["pages/__init__.py<br/>build_router()"]
Build --> Import["导入所有资源模块"]
Import --> Auth["auth.py"]
Import --> Dashboard["dashboard.py"]
Import --> Users["users.py"]
Import --> Conversations["conversations.py"]
Import --> Memories["memories.py"]
Import --> Knowledge["knowledge.py"]
Import --> Skills["skills.py"]
Import --> Scenes["scenes.py"]
Import --> Settings["settings.py"]
Import --> AdminUsers["admin_users.py"]
Import --> Audit["audit.py"]
Import --> Export["export.py"]
Import --> CounselingDashboard["counseling_dashboard.py"]
Import --> Counselors["counselors.py"]
Import --> Appointments["appointments.py"]
Import --> Orders["orders.py"]
Auth --> Shared["_shared.py<br/>共享基础设施"]
Dashboard --> Shared
Users --> Shared
...
### 共享基础设施
**新增** _shared.py 模块提供了管理后台的核心基础设施：

- **模板引擎**：统一的 Jinja2Templates 配置，避免路径问题
- **鉴权依赖**：require_admin_page() 和 require_admin_api() 提供统一的鉴权逻辑
- **统计上下文**：get_stats_context() 提供仪表盘统计数据
- **通用模型**：BatchDeleteRequest、LoginRequest 等 Pydantic 模型
- **工具函数**：login_redirect() 提供统一的重定向处理
...
## 依赖关系分析
- **路由聚合**
  - admin/__init__.py 作为兼容层，通过 build_router() 暴露 router
  - pages/__init__.py 将各资源模块路由聚合至 /admin
- **鉴权依赖**
  - 页面与 API 均依赖 auth.py 提供的管理员解析与 CSRF 校验
  - 页面模块通过 _shared.py 统一使用 require_admin_page() 和 require_admin_api()
- **数据层依赖**
...
```mermaid
graph LR
Init["admin/__init__.py<br/>兼容层"] --> PagesInit["pages/__init__.py<br/>路由聚合"]
PagesInit --> Auth["auth.py"]
PagesInit --> Dashboard["dashboard.py"]
...
PagesInit --> AdminUsers["admin_users.py"]
PagesInit --> CounselingDashboard["counseling_dashboard.py"]
PagesInit --> Counselors["counselors.py"]
PagesInit --> Appointments["appointments.py"]
PagesInit --> Orders["orders.py"]
PagesInit --> Scenes["scenes.py"]
Auth --> Shared["_shared.py<br/>共享基础设施"]
Dashboard --> Shared
Users --> Shared
Conversations --> Shared
...
## 结论
本管理后台在安全性、可追溯性与可扩展性方面提供了完善的基础能力：管理员令牌与 CSRF 双重保障、全链路审计日志、统一的导出工具与分页查询。**新的模块化架构**进一步提升了代码的可维护性和扩展性，通过共享基础设施消除了重复代码，使每个资源模块更加独立和专注。建议在后续迭代中补充更细粒度的 RBAC、速率限制与监控告警指标采集，以满足更大规模运营需求。
