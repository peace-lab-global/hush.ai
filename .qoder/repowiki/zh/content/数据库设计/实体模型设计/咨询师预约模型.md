...
- [counselors.py](file://hushai/meditation/admin/pages/counselors.py)
- [appointments.py](file://hushai/meditation/admin/pages/appointments.py)
</cite>

## 更新摘要
**变更内容**   
- 新增完整的咨询服务数据库模型支持
- 实现咨询师资质管理系统（专业领域、资质证书、评级体系）
- 构建排班时段管理功能（CounselorSchedule）
- 完善预约流程完整生命周期（Appointment）
- 集成微信支付订单系统（ConsultationOrder）
- 实现敏感数据加密存储（ServiceRecord）
- 提供预约配置管理（AppointmentSettings）
...
- 管理后台页面：hushai/meditation/admin/pages/counselors.py、hushai/meditation/admin/pages/appointments.py

```mermaid
graph TB
subgraph "API 层"
A["counseling.py<br/>用户端预约/订单/支付"]
B["counselor_dashboard.py<br/>咨询师后台/服务记录"]
C["admin pages<br/>管理后台"]
end
subgraph "领域模型"
...
- **状态管理**：pending/approved/rejected/disabled，影响是否可被预约
...
- [counselor_dashboard.py:214-274](file://hushai/meditation/api/counselor_dashboard.py#L214-L274)
...
- **数据库索引优化**
  - 充分利用现有索引：counselor_id、schedule_date、appointment_date、status 等常用查询字段
...
- **咨询师不可见**
  - 现象：咨询师列表中没有显示特定咨询师
  - 排查：确认咨询师状态为 approved 且 is_online 为 True
  - 参考：[counseling.py:76](file://hushai/meditation/api/counseling.py#L76)
...
## 结论
本模型围绕咨询师资质、排班、预约、订单、服务记录与配置形成闭环，结合微信支付与敏感数据加密，满足心理咨询服务的合规性与用户体验需求。系统在数据安全、业务流程完整性方面表现良好，建议在后续迭代中完善排班容量控制、支付幂等与数据库级唯一约束，进一步提升系统健壮性。
