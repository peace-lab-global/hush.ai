...
B["pages/_shared.py<br/>模板/统计/鉴权辅助"]
end
subgraph "页面路由(示例)"
C["pages/dashboard.py"]
D["pages/users.py"]
E["pages/knowledge.py"]
F["pages/skills.py"]
G["pages/scenes.py"]
H["pages/settings.py"]
end
subgraph "认证与安全"
I["auth.py<br/>JWT/CSRF/管理员校验"]
end
subgraph "数据模型"
J["db/models.py<br/>User/Skill/KnowledgeChunk/Scene 等"]
end
subgraph "前端模板与样式"
K["templates/base.html<br/>侧边栏/工具栏/通用JS"]
L["static/css/admin.css<br/>主题/布局/组件样式"]
end
A --> C
A --> D
A --> E
...
## 结论
管理后台以清晰的模块化组织与统一的模板/样式体系为基础，提供了用户管理、内容管理、系统设置等核心能力。通过标准化的鉴权与 CSRF 保护、完善的分页与搜索、以及一致的交互反馈，保证了良好的可用性与可维护性。后续可在仪表盘中引入更多可视化图表与趋势分析，并在导出与审计方面进一步增强。
